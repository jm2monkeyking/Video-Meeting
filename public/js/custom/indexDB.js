...
openRequest.addEventListener("upgradeneeded", (e) => {
  ...
  const objectStore = db.createObjectStore("video_os", {
    keyPath: "id",
    autoIncrement: true,
  });
...
function addVideoToDB(blob_array, callback) {
  const newItem = { blob: blob_array, name: new Date().getTime(), sync: 0 };
  // open a read/write db transaction, ready for adding the data
  const transaction = db.transaction(["video_os"], "readwrite");

  // call an object store that's already been added to the database
  const objectStore = transaction.objectStore("video_os");
...
function getAllVideo() {
  ...
  const objectStore = db.transaction("video_os").objectStore("video_os");

  var getAllRequest = objectStore.getAll();
  getAllRequest.onsuccess = function () {
    // Get a reference to the cursor
    // const cursor = getAllRequest.result;
    // con;
    return getAllRequest.result.length > 0 ? getAllRequest.result : [];
